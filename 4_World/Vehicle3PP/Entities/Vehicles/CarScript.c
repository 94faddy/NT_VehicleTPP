modded class CarScript
{
	...
	override void EEInit()
	{
		...
		if (GetGame().IsServer() || GetGame().IsDedicatedServer())
		{
			...
			if (!config)
			{
				Print("[VehicleTPP] ERROR: Config is null in CarScript.EEInit()");
				return;
			}
			...
			else
			{
				...
				foreach (string allowedVehicle : vehicles)
				{
					...
					{
						m_Allow3PPCamera = true;
						Print("[VehicleTPP] Allowing 3PP for vehicle: " + vehicleType);
						break;
					}
				}
				...
				if (!m_Allow3PPCamera)
				{
					Print("[VehicleTPP] Denying 3PP for vehicle: " + vehicleType);
				}
			}
